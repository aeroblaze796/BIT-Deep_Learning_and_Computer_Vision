...
class Config:
    ...
    RESULTS_DIR = '演示性Baseline-ResNet18-加权损失函数_结果'
...
if __name__ == '__main__':
    config = Config()
    # 这里需要从 get_dataloaders 返回 image_datasets 来获取类别统计信息
    def get_dataloaders_and_datasets(config):
        data_transforms = {
            'train': transforms.Compose([
                transforms.RandomResizedCrop(224),
                transforms.RandomHorizontalFlip(),
    ...
    dataloaders, dataset_sizes, class_names, image_datasets = get_dataloaders_and_datasets(config)
