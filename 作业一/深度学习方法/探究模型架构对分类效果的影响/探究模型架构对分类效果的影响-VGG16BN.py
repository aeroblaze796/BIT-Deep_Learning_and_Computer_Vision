...
class Config:
    ...
    LEARNING_RATE = 0.0001 # VGG对学习率更敏感，使用一个较小的值
    
    EARLY_STOPPING_PATIENCE = 20

    RESULTS_DIR = '探究模型架构对分类效果的影响-VGG16BN'
...
def save_and_plot_results(history, best_model, config):
    ...
    model_save_path = os.path.join(config.RESULTS_DIR, 'best_vgg16bn_model.pth')
    ...
    plt.suptitle('VGG16-BN Model Training History')
    plt.tight_layout(rect=[0, 0, 1, 0.96])

    plot_save_path = os.path.join(config.RESULTS_DIR, 'training_history_vgg16bn.png')
...
def print_summary_report(model, history, best_test_acc, final_epoch, time_elapsed):
    print("\n" + "="*30)
    print("      VGG16-BN Experiment Summary")
    print("="*30)
...
if __name__ == '__main__':
    ...
    # 替换 VGG 的分类头
    # VGG的分类器是一个nn.Sequential，我们需要替换它的最后一个全连接层
    # 打印 model.classifier 结构可以看到，最后一个层是索引为6的nn.Linear
